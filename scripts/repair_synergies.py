"""
Repair Synergies Script for LeagueStats Coach.

This script detects and repairs champions that are missing synergy data in the
database. It performs targeted re-scraping of only the affected champions to
avoid destroying existing data.

CRITICAL SAFETY RULES:
- NEVER calls parse_all_synergies() -- would DROP entire synergies table
- NEVER calls init_synergies_table()  -- would DROP entire synergies table
- Only calls clear_synergies_for_champion() + add_synergies_batch() per champion
...
USAGE:
    ...
    python scripts/repair_synergies.py --max-workers 3
...
def _setup_logging(log_to_file: bool = True) -> logging.Logger:
    ...
    Args:
        log_to_file: If True, also write logs to logs/repair_synergies.log
    ...
    if log_to_file:
        ...
        log_file = log_dir / "repair_synergies.log"
...
def detect_champions_without_synergies(db: Database) -> List[str]:
    """Find all champions that have zero rows in the synergies table.

    Uses a LEFT JOIN so champions absent from synergies are identified even
    if the synergies table is empty or missing rows.

    Args:
        db: Connected Database instance

    Returns:
        Sorted list of champion names with no synergy data
    """
...
def _scrape_champion_synergies(
    champion: str,
    patch_version: str,
    headless: bool,
) -> Tuple[str, list]:
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Repair champions missing synergy data in the LeagueStats database. "
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print champions missing synergies without scraping anything",
    )
    parser.add_argument(
        "--max-workers",
        type=int,
...
def main() -> int:
    ...
    logger.info("=" * 70)
    logger.info("LeagueStats Coach - Repair Synergies")
    logger.info("=" * 70)
    ...
    try:
        ...
        # ---- Detect champions without synergies ----
        logger.info("Detecting champions without synergy data...")
        missing = detect_champions_without_synergies(db)

        if not missing:
            logger.info("All champions have synergy data. Nothing to repair.")
            return 0

        logger.info(f"Found {len(missing)} champion(s) without synergies:")
        for name in missing:
            logger.info(f"  - {name}")
