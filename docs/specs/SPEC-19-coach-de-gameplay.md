...
**Statut** : 📝 Brouillon, affiné avec @pj35 le 2026-09-26. **À valider.** Phase 0 (spike) prête
à lancer (`scripts/spike_gameplay_dump.py`) ; les phases 1 à 4 seront précisées avec ses résultats.
...
**Effort** : ~6 jours sur quatre phases (§11), à redécouper après le spike.
...
devenir les fixtures des tests. Les lancer aussi une fois pendant l'écran de fin, pour la
question 5.
...
Chaque métrique se lit contre l'une de trois références, par ordre de priorité d'affichage :
...
| **Objectif** | Où est le niveau visé ? | OneTricks `matchHistory` (Master+, **ton champion**) pour les métriques qu'il publie ; sinon une valeur repère par rôle, sourcée (§5.2) ; ou un objectif fixé par le joueur | Dès la phase 3 |
...
1. **Valeur** `v` de la partie, **référence** `r` et **dispersion** `s` : moyenne et écart-type
   de la norme quand elle compte au moins `COACHING_MIN_NORM_SAMPLE` valeurs, sinon objectif et
   écart relatif.
2. **Écart orienté** `z = sens(m) × (v − r) / s`, où `sens` vaut +1 si « plus » est mieux (CS),
   −1 sinon (morts). Un `z` positif est toujours une bonne nouvelle.
...
- **Récurrence** : un constat négatif (ou positif) devient un **schéma** quand sa fréquence sur
  les `COACHING_RECURRENCE_WINDOW` dernières parties dépasse significativement son taux de base.
  Même test binomial que SPEC-15 §3.2.1 (`math.comb`, stdlib), même seuil α en configuration.
- **Tendance** : moyenne glissante par métrique, et verdict « progrès », « recul » ou « stable »
  seulement quand l'écart dépasse le bruit. Un CUSUM est à évaluer en phase 4. Pas de verdict
  sous `COACHING_MIN_TREND_SAMPLE` parties : l'ignorance reste visible (esprit SPEC-09).
- **Axes de travail** : **un ou deux actifs** à la fois. Soit le joueur les fixe, soit ils sont
  proposés à partir du schéma négatif le plus fort. Ensuite :
  - **rappel en draft**, une ligne à l'écran de fin de draft ;
  - **verdict en fin de partie** sur l'axe ;
...
- **Bilan** : une commande du Live Coach et une entrée de menu résument la progression par axe
  et par métrique principale. En console d'abord, en GUI quand elle existera.

## 8. Stockage et modules

**Migration Alembic** (numéro de révision au moment de l'écrire) :

- `game_records` : `game_id` (PK), `queue_id`, `game_creation_utc`, `duration_s`,
  `player_participant_id`, `raw_game` (JSON), `raw_timeline` (JSON, NULL si indisponible),
  `captured_utc`. Le brut est la source de vérité, et les métriques en sont dérivées.
- `game_metrics` (format long) : `game_id`, `participant_id`, `role`, `champion_id`, `metric`,
  `value`. Elle est **recalculable** depuis `game_records`, puisqu'une nouvelle métrique ne doit
  pas exiger de rejouer les parties. Les 10 participants y sont : c'est la norme de §4.
- `coaching_goals` (phase 4) : axes de travail, dates, statut.
...
| `progression.py` | Récurrence, tendances, axes de travail (phase 4) |
...
| 29 | Migration `game_records` + `game_metrics`, repository et délégué `db.py` | 1 | 2 | 28 |
...
| 34 | `findings.py` : écarts orientés et classement | 3 | 3 | 33 |
| 35 | `report.py` : rapport console en fin de partie, ligne d'attendu de la draft | 3 | 2 | 34 |
| 36 | `progression.py` : récurrence et tendances | 4 | 3 | 34 |
| 37 | Axes de travail : `coaching_goals`, rappel en draft, verdict, acquisition | 4 | 5 | 36 |
| 38 | Bilan de progression (commande du Live Coach et menu) | 4 | 3 | 36 |

**Ordre** : 29-30 dès le spike rendu, pour que la capture tourne pendant qu'on explore. La
...
3. `game_metrics` se recalcule intégralement depuis `game_records`, sans appel LCU.
