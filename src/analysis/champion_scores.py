...
class GlobalScoreCalculator:
    """Compute and persist per-(champion, lane) scores from raw matchup data."""
    ...
    def _score_champion(self, champion: str, champion_id: int, lane: Optional[str]) -> bool:
        """Compute and save one (champion, lane) score row.

        lane=None fetches the toutes-lanes aggregate (saved under
        analysis_config.ALL_LANES_KEY); a scraping_config.LANES value scopes
        the underlying matchups to that lane only.

        Returns True if a score was saved, False if skipped (no data for
        this champion/lane combination).
        """
        matchups = self.db.get_champion_matchups_by_name(champion, lane=lane)
        if not matchups:
            if self.verbose:
                print(f"  [SKIP] {champion} ({lane or 'all'}): No matchups found")
            return False

        valid_matchups = self.scorer.filter_valid_matchups(matchups)
        if not valid_matchups:
            if self.verbose:
                print(f"  [SKIP] {champion} ({lane or 'all'}): No valid matchups after filtering")
            return False
        ...
        self.db.save_champion_scores(
            ...
            lane=lane or analysis_config.ALL_LANES_KEY,
        )

        if self.verbose:
            print(
                f"  [OK] {champion} ({lane or 'all'}): avg_delta2={avg_delta2:.3f}, "
                f"variance={variance:.3f}, coverage={coverage:.3f}"
            )
        return True
    ...
    def calculate_all(self) -> int:
        ...
        peak_impact, volatility, target_ratio) for all champions, both as a
        toutes-lanes aggregate and scoped to each lane the champion has
        matchup data for, and stores them in the champion_scores table.
        ...
        for champion in all_champions:
            try:
                ...
                if self._score_champion(champion, champion_id, lane=None):
                    rows_scored += 1

                for lane in scraping_config.LANES:
                    if self._score_champion(champion, champion_id, lane=lane):
                        rows_scored += 1
        ...
        print(
            f"[SUCCESS] Scored {rows_scored} (champion, lane) rows "
            f"across {len(all_champions)} champions"
        )
        return rows_scored
