...
class TestTierListGeneration:
    ...
    def test_generate_blind_pick_tier_list(self, assistant, sample_champions):
        ...
        champions_with_scores = [c for c in champion_pool if c in ['Aatrox', 'Ahri', 'Jinx', 'Thresh', 'Lee Sin']]
        assert len(tier_list) == len(champions_with_scores), f"Should have {len(champions_with_scores)} entries"
...
class TestTierListNonRegression:
    ...
    def test_method_exists_on_assistant(self, assistant):
        ...
        assert hasattr(assistant, 'generate_tier_list'), \
    ...
    def test_method_callable(self, assistant):
        ...
        assert callable(assistant.generate_tier_list), \
            "generate_tier_list must be callable"

    def test_delegation_to_tier_list_generator(self, assistant):
        """Regression test: Ensure Assistant delegates to TierListGenerator."""
        assert hasattr(assistant, 'tier_list_gen'), \
            "Assistant must have tier_list_gen attribute"
        assert hasattr(assistant.tier_list_gen, 'generate_tier_list'), \
