...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("fill_db.log", encoding="utf-8"),
    ],
)
...
def main():
    ...
    logger.info("Patch: %s | Workers: %d | Headless: %s", PATCH, MAX_WORKERS, scraping_config.HEADLESS)
    ...
    c = conn.cursor()
    c.execute("SELECT COUNT(*) FROM champions"); champs = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM matchups"); m = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM synergies"); s = c.fetchone()[0]
    conn.close()
